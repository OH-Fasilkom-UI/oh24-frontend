...
const HeroSection = () => {
  ...
  const [breakpoint, setBreakpoint] = useState<keyof typeof BREAKPOINT_CONFIGS>(
    getBreakpoint(windowWidth)
  )
  // Minimal satu panda ada
  const calculatePandasToShow = (
    participants: number,
    maxPandas: number
  ): number => {
  ...
  const pandasToShow = calculatePandasToShow(
    jumlahParticipants,
    config.maxPandas
  )
  ...
  return (
    <section className="w-full flex flex-col items-center pt-[90px] max-md:pt-[70px]">
      <h1
        className="text-[80px] max-md:text-[40px] font-riffic text-BlueRegion/GovernorBay/600 text-center md:whitespace-nowrap"
        style={{
          textShadow: '0px 3px 3px rgba(46,56,129,0.5)',
        }}
      >
        Get your{' '}
        <span
          className="font-riffic text-BlueRegion/GovernorBay/600 max-md:text-[70px] max-md:uppercase max-md:leading-none"
          style={{
            textShadow: '0px 3px 3px rgba(46,56,129,0.5)',
          }}
        >
      ...
      <h1
        className="-mt-1 md:-mt-2.5 text-[56px] max-md:text-[40px] font-riffic text-BlueRegion/GovernorBay/600 text-center md:whitespace-nowrap max-md:leading-tight"
        style={{
          textShadow: '0px 3px 3px rgba(46,56,129,0.5)',
        }}
      >
      ...
      {/* First Layer */}
      <div className="flex flex-wrap justify-center gap-[3rem] w-full mt-6">
      ...
      {940 - jumlahParticipants === 0 ? (
        <h2 className="text-t6 max-sm:text-t7 text-BlueRegion/CeruleanBlue/700 text-center font-tex-gyre font-bold mt-8 px-2">
          ...
          <br />
          Yuk segera daftar Online Experience!
        </h2>
      ) : (
        <h2 className="text-t6 max-sm:text-t7 text-BlueRegion/CeruleanBlue/700 text-center font-tex-gyre font-bold mt-8 px-2 max-md:px-8">
          Yuk! Masih ada {940 - jumlahParticipants} tempat tersedia untuk Full
          Experience Open House Fasilkom UI!
          <br />
