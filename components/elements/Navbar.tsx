          ...
          <>
            {NAVBAR_LINKS.map((link, index) => (
              <div
                ...
                className={`items-center text-[16px] max-xl:text-[12px] max-lg:hidden gap-2 flex font-tex-gyre font-semibold  ${link.isExist
                    ? pathname === link.href
              ...
              <PopoverContent className="w-fit h-fit p-5 flex flex-col rounded-[8px] gap-5 bg-[#f3f3f3]">
                {NAVBAR_LINKS.map((link, index) => (
                  <p
                    ...
                    className={`font-tex-gyre flex cursor-pointer whitespace-nowrap flex-row items-center gap-4 text-[12px] justify-start ${link.isExist
                        ? pathname === link.href
